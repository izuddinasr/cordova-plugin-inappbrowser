...
- __options__: Options for the `InAppBrowser`. Optional, defaulting to: `location=yes`. _(String)_
    ...
    - __beforeclose__: set to enable the `beforeclose` event when window is about to close. Accepted values is `yes` or `no` to trigger the event. Default is set to `no`
...
- __eventname__: the event to listen for _(String)_
  ...
  - __beforeclose__: event fires when the `InAppBrowser` window is about to close.
